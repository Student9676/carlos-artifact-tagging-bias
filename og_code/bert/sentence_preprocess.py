...
for (
    ...
    jargon_labels_array,
    social_labels_array,
    annotated_data["ObjectID"],
    annotated_data["Subjective"],
    annotated_data["Gender"],
    annotated_data["Jargon"],
    annotated_data["Social"],
):
    ...
    for sentence in sentences:
        ...
        matching_subjective = [
            lbl
            for lbl in subjective_labels
            if lbl.lower().replace('"', "") in sentence.lower()
        ]
        matching_gender = [
            lbl
        ...
        matching_jargon = [
            lbl
            for lbl in jargon_labels
            if lbl.lower().replace('"', "") in sentence.lower()
        ]
        matching_social = [
            lbl
            for lbl in social_labels
            if lbl.lower().replace('"', "") in sentence.lower()
        ]
        ...
        new_rows.append(
            {
                ...
                "Subjective Label": (
                    matching_subjective if matching_subjective else None
                ),
...
sentence_level_data.to_csv(
    "carlos_data/sentence_level_preprocessed_data.csv", index=False, encoding="utf-8"
)

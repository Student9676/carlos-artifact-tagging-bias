...
def extract_bias_labels(response_text):
    ...
    keywords = {
        ...
        "gender bias": [],
        "jargon bias": ["jargon", "technical", "terms", "typology", "appliqué", "pestle", "iconography", "provenance", "acquisition"],
...
def process_excel(file_path, api_key, model_id, object_ids):
    ...
    for index, row in df.iterrows():
        ...
        print(f"Extracted bias labels: {bias_labels}")  # Debug: Print the extracted bias labels
        binary_result = convert_bias_labels(bias_labels)

        print(f"Processed row {index + 1}/{len(df)}: {binary_result}")
        print(f"Original API Response: {response_text}")  # Print the response text from the API
...
def main():
    ...
    while True:
        ...
        input_object_ids = input(
            "Enter ObjectID or range of ObjectIDs (e.g., '1' or '1-5') or 'exit' to stop: ").strip()
